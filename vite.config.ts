...
react(),
    ...
    resolve: {
      alias: {
        ...
        '@dog': path.resolve(__dirname, './dog race'),
      },
    },
    server: {
      ...
      strictPort: true,
      // The Spin & Wheel API + WebSocket run as a separate Node process (pnpm dev:server)
      proxy: {
        '/api': { target: `http://localhost:${process.env.SERVER_PORT || 8787}`, changeOrigin: false },
        '/ws': { target: `ws://localhost:${process.env.SERVER_PORT || 8787}`, ws: true },
      },
      watch: {
        ignored: [
          '**/.figma/**',
